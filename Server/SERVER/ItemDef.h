...
// ItemID에서 부위와 단계를 역산한다. 방어구가 아니면 false.
...
// 최종 피해 = floor(dmg * (100 - 헬멧 - 상의) / 100), 명중이면 최소 1.
// 정수 나눗셈이 곧 버림이므로 부동소수를 쓰지 않는다.
inline short ApplyArmorReduction(short dmg, int helmet_grade, int body_grade)
{
	if (dmg <= 0) return 0;   // 빗나감을 최소 1 보정으로 명중시키지 않기 위한 가드
