...
bool NetworkManager::SendHitNpc(const DirectX::XMFLOAT3& rayOrigin, const DirectX::XMFLOAT3& rayDirection, short weaponType, short weaponGrade)
{
...
bool NetworkManager::SendHitPlayer(const DirectX::XMFLOAT3& rayOrigin, const DirectX::XMFLOAT3& rayDirection, short weaponType, short weaponGrade)
{
	...
	ZeroMemory(&pkt, sizeof(pkt));
	pkt.size = sizeof(CS_HIT_PLAYER_PACKET);
	pkt.type = CS_HIT_PLAYER;
	pkt.ray_ox = rayOrigin.x;
	pkt.ray_oy = rayOrigin.y;
	pkt.ray_oz = rayOrigin.z;
	pkt.ray_dx = rayDirection.x;
...
bool NetworkManager::SendRoundJoin()
{
	...
	ZeroMemory(&pkt, sizeof(pkt));
	pkt.size = sizeof(CS_ROUND_JOIN_PACKET);
	pkt.type = CS_ROUND_JOIN;
