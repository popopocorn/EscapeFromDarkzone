...
void CShader::CreateShader(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature)
{
	...
	m_d3dPipelineStateDesc.NumRenderTargets = 3;
...
void UIObjectShader::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera, bool batch, int nPipelineState)
{
	OnPrepareRender(pd3dCommandList, nPipelineState);
...
void FullscreenShader::CreateShader(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature)
{
	::ZeroMemory(&m_d3dPipelineStateDesc, sizeof(D3D12_GRAPHICS_PIPELINE_STATE_DESC));
	m_d3dPipelineStateDesc.pRootSignature = pd3dGraphicsRootSignature;
	m_d3dPipelineStateDesc.VS = CreateVertexShader();
...
{
	return CShader::CompileShaderFromFile(L"Shaders.hlsl", "VSFullScreen", "vs_5_1", &m_pd3dVertexShaderBlob);
}

D3D12_SHADER_BYTECODE FullscreenShader::CreatePixelShader()
{
	return CShader::CompileShaderFromFile(L"Shaders.hlsl", "PSFullScreen", "ps_5_1", &m_pd3dPixelShaderBlob);
}

D3D12_DEPTH_STENCIL_DESC FullscreenShader::CreateDepthStencilState()
{
	D3D12_DEPTH_STENCIL_DESC d3dDepthStencilDesc;
	::ZeroMemory(&d3dDepthStencilDesc, sizeof(D3D12_DEPTH_STENCIL_DESC));
	d3dDepthStencilDesc.DepthEnable = FALSE;
...
D3D12_SHADER_BYTECODE Lighting::CreatePixelShader()
{
