...
bool CGameFramework::OnCreate(HINSTANCE hInstance, HWND hMainWnd)
{
	...
	// 03.27 Ï∂îÍ∞Ä: ÎÑ§Ìä∏ÏõåÌÅ¨ Ï¥àÍ∏∞Ìôî Î∞è Ïó∞Í≤∞
...
void CGameFramework::CreateDirect3DDevice()
{
	...
	{
		// Auto-BreadcrumbsøÕ Page Fault √ﬂ¿˚ ∞≠¡¶ »∞º∫»≠
...
void CGameFramework::FrameAdvance()
{
	...
	// 03.27 Ï∂îÍ∞Ä, 03.30 ÏúÑÏπò Î≥ÄÍ≤Ω
	...
	if (hr == DXGI_ERROR_DEVICE_REMOVED || hr == DXGI_ERROR_DEVICE_RESET)
	{
		...
		OutputDebugStringA("GPU ≈©∑°Ω√(Device Removed) πﬂª˝!\n");

		// 2. DRED µ•¿Ã≈Õ √ﬂ√‚
		ComPtr<ID3D12DeviceRemovedExtendedData1> pDred;
		if (SUCCEEDED(m_pd3dDevice->QueryInterface(IID_PPV_ARGS(&pDred)))) // µπŸ¿ÃΩ∫ø°º≠ DRED ¿Œ≈Õ∆‰¿ÃΩ∫ ∞°¡Æø¿±‚
		{
			D3D12_DRED_AUTO_BREADCRUMBS_OUTPUT1 DredAutoBreadcrumbsOutput;
			D3D12_DRED_PAGE_FAULT_OUTPUT DredPageFaultOutput;

			// ∏Ì∑…æÓ Ω««‡ ±‚∑œ(Breadcrumbs) π◊ ∆‰¿Ã¡ˆ ∆˙∆Æ µ•¿Ã≈Õ ∞°¡Æø¿±‚
			pDred->GetAutoBreadcrumbsOutput1(&DredAutoBreadcrumbsOutput);
			pDred->GetPageFaultAllocationOutput(&DredPageFaultOutput);

			// 3. µπˆ∞≈∏¶ ∞≠¡¶∑Œ ∏ÿ√ﬂ∞‘ «‘
			// ø©±‚º≠ ¡ﬂ¥‹¡°(Breakpoint)¿Ã ∞…∏Æ∏È, Visual Studio¿« '¡∂ªÁΩƒ(Watch)' √¢ø°º≠
			// DredAutoBreadcrumbsOutput ∫Øºˆ∏¶ ø≠æÓ æÓ∂≤ ≥ÎµÂ(∏Ì∑…æÓ)±Ó¡ˆ Ω««‡µ«¥Ÿ ¡◊æ˙¥¬¡ˆ »Æ¿Œ«’¥œ¥Ÿ.
			__debugbreak();
		}
	}
#endif
...
void CGameFramework::ProcessNetworkPackets()
{
	...
	while (true)
	{
		...
		char type = packet[1]; // Ìå®ÌÇ∑ ÌÉÄÏûÖ ÌôïÏù∏
		switch (type)
		{
		case SC_LOGIN_INFO:
		{
			...
			// 03.30 Ï∂îÍ∞Ä: ÏÑúÎ≤ÑÎ°úÎ∂ÄÌÑ∞ Î∞õÏïÑÏò® Ï¥àÍ∏∞ ÏúÑÏπòÎ°ú Ïù¥Îèô
			m_pPlayer->SetPosition(XMFLOAT3(p->x, p->y, p->z));
			m_pPlayer->SetServerPosition(XMFLOAT3(p->x, p->y, p->z)); // 04.10 Ï∂îÍ∞Ä: ÏÑúÎ≤Ñ ÏúÑÏπò Ï¥àÍ∏∞Ìôî
			m_myId = p->id; // 03.30 Ï∂îÍ∞Ä: ÎÇ¥ ID Ï†ÄÏû•
			break;
		}
		case SC_ADD_PLAYER:
		{
			...
			// 03.30 Ï∂îÍ∞Ä: ÏûòÎ™ªÎêú Ìå®ÌÇ∑ ÎÑòÍ∏∞Í∏∞ (Broadcast Í¥ÄÎ†® Ïò§Î•ò?)
			if (p->id == m_myId) {
				break;
			}

			//03.30 Ï∂îÍ∞Ä: OtherPlayer ÏÉùÏÑ±, 04.07 ÏàòÏ†ï: Ìï®Ïàò ÎûòÌïë
			if (m_otherPlayers.count(p->id)) break;		// Ïù¥ÎØ∏ ÏûàÏúºÎ©¥ ÎÑòÍ∏∞Í∏∞ (Ïò§Î©¥ ÏïàÎêòÎäî Ìå®ÌÇ∑)
		...
		case SC_REMOVE_PLAYER:
		{
			...
			// 03.30 Ï∂îÍ∞Ä: OtherPlayer Ï†úÍ±∞
			auto it = m_otherPlayers.find(p->id);
			if (it != m_otherPlayers.end()) {
				it->second->Kill();			// ÏûÑÏãú ÏÇ≠Ï†ú, Ï∂îÌõÑ ÏàòÏ†ï ÌïÑÏöî
		...
		case SC_MOVE_PLAYER:
		{
			...
			// 03.30 Ï∂îÍ∞Ä: OtherPlayer ÏúÑÏπò ÏóÖÎç∞Ïù¥Ìä∏ (ÌîåÎ†àÏù¥Ïñ¥ ÏúÑÏπò Î≥¥Ï†ï ÎØ∏Íµ¨ÌòÑ)
			if (p->id == m_myId) {
				m_pPlayer->SetServerPosition(XMFLOAT3(p->x, p->y, p->z));	// 04.10 Ï∂îÍ∞Ä: Î≥¥Í∞ÑÏö© ÏÑúÎ≤Ñ ÏúÑÏπò
				break;
			}
